...
## 10. 🔊 تصميم الصوت التفاعلي
لتحسين الشعور الملموس والمكافأة البصرية عند إكمال المهام، قمنا بدمج نغمة تنبيه صوتية مخصصة.
- **الميزات:**
  - **تغذية صوتية ممتازة:** تشغيل نغمة تتابعية (C6→E6 arpeggio) مكونة من نغمتين عند إكمال المهام (مستوحاة من أدوات الإنتاجية الحديثة مثل TickTick).
  - **حماية من التكرار السريع (Debounce):** تنفيذ حاجز زمني مدته 500 ملي ثانية لمنع تداخل الموجات الصوتية عند إنجاز عدة مهام متتالية بسرعة فائقة.
  - **التوافق مع سياسات التشغيل التلقائي (Autoplay):** حل قيود المتصفحات على أجهزة iOS وديسكتوب من خلال تسجيل مستمع تفاعلي لمرة واحدة على مستوى المستند لاستئناف الـ `AudioContext` تلقائياً عند أول تفاعل.
  - **حفظ تفضيلات الصوت:** ربط الميزة بقاعدة البيانات عبر حقل (`soundEnabled`) لحفظ رغبة المستخدم عبر الأجهزة ومساحات العمل.
- **الكود:** `src/lib/audio.ts` & `src/app/api/user/profile/route.ts`

<br/>

## 11. 📊 لوحة العمل التفاعلية التجريبية (صفحة الهبوط)
قبل أن يقرر المستخدم تسجيل حساب جديد أو بدء جلسة ضيف، توفر صفحة الهبوط لوحة تفاعلية تجريبية بالكامل تعمل في جهة العميل لمحاكاة الداشبورد.
- **الميزات:**
  - **تخصيص الأقسام ديناميكياً:** تتيح للزوار إخفاء وتفعيل الوحدات المختلفة (المهام، المشاريع، التقويم، الملاحظات) ورؤية التغير الفوري.
  - **حركات Framer Motion:** استخدام الـ `LayoutGroup` لتنظيم انتقالات سلسة وإعادة تموضع الكروت تلقائياً وتفادي الفراغات.
  - **صوت وجسيمات تفاعلية:** النقر على المهام التجريبية يشغل نغمة الإنجاز الفورية ويطلق 12 جسيماً ملوناً ومضيئاً من مركز النقر بتأثير مبهر.
  - **مؤشر تقدم المشاريع:** دائرة مؤشر SVG تترجم نسبة إنجاز المهام الثلاثة التجريبية ديناميكياً من 0% إلى 100%.
  - **عناصر تحكم macOS:** تصميم شريط علوي يحاكي نظام macOS باتجاه ثابت LTR لجمالية فائقة.
- **الكود:** `src/components/home/hero-playground.tsx`
